...
2. The VEML6070 sensor's drivers are already loaded onto your ESP32. Let's test to make sure that everything works.
    i. The first step is to initialize the I<sup>2</sup>C interface and initialize the VEML6070 drivers.
      ```
      i2c = I2C(scl=Pin(5), sda=Pin(4))
      uv = veml6070.VEML6070(i2c)
      ```
    ii. After initialization, we can print out the UV index values at regular intervals:
      ```
      while True:
          uv_raw = uv.uv_raw
